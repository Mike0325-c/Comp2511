...
public class BattleTest {
    ...
    @Test
    @Tag("11-6")
    @DisplayName("Test player battles hydra and hydra dies")
    ...
    public void testHydraDiesWhenBattle() {
        DungeonManiaController controller = new DungeonManiaController();
        DungeonResponse postBattleResponse = TestUtils.genericHydraSequence(
                controller, "c_battleTests_basicHydraHydraDies");
        List<EntityResponse> entities = postBattleResponse.getEntities();
    ...
    @Test
    @Tag("11-25")
    @DisplayName("Test a player wins a battle against a hydra with an invincibility potion")
    public void testPlayerWinsHydraBattleWithInvincibilityPotion() throws InvalidActionException {
        DungeonManiaController controller = new DungeonManiaController();
        String config = "c_battleTests_battleWithInvincibility";
        controller.newGame("d_battleTest_invincibleHydra", "c_battleTests_battleWithInvincibility");
        ...
        // One hit kill = should be entire health
        int enemyHealth = Integer.parseInt(TestUtils.getValueFromConfigFile("hydra_health", config));
        assertEquals(0, battle.getRounds().get(0).getDeltaCharacterHealth(), 0.001);
        // Delta health is negative so take negative here
        assertTrue(-battle.getRounds().get(0).getDeltaEnemyHealth() >= enemyHealth);
    }
    ...
    public void testHealthBelowZeroHydra() {
        ...
        DungeonResponse postBattleResponse = TestUtils.genericHydraSequence(
                controller, "c_battleTests_basicHydraPlayerDies");
        BattleResponse battle = postBattleResponse.getBattles().get(0);
        assertBattleCalculations(battle, false, "c_battleTests_basicHydraPlayerDies", "hydra");
    }
