...
public class EntityFactory {
    ...
    public Hydra buildHydra(Position pos) {
        double hydraHealth = config.optDouble("hydra_health");
        double hydraAttack = config.optDouble("hydra_attack");
        double healthIncreaseAmount = config.optDouble("hydra_health_increase_amount");
        double healthIncreaseRate = config.optDouble("hydra_health_increase_rate");
        return new Hydra(pos, hydraHealth, hydraAttack, healthIncreaseAmount, healthIncreaseRate);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        switch (jsonEntity.getString("type")) {
            ...
            case "swamp_tile":
                int movementFactor = config.optInt("movement_factor", 1);
