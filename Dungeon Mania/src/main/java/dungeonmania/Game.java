...
public class Game {
    ...
    public Game build(String buildable) throws InvalidActionException {
        ...
        if (buildables.get(buildables.indexOf(buildable)).equals("midnight_armour") && getZombieToastCount() > 0) {
            throw new InvalidActionException(String.format("%s cannot be built", buildable));
        }
    ...
    public Position getMapDijkstra(Entity entity) {
        return map.dijkstraPathFind(entity.getPosition(), map.getPlayer().getPosition(), entity);
    }
    ...
    public void getEntityFactorySpawn(Game game, ZombieToastSpawner spawner) {
    ...
    public boolean areAllBouldersActivated() {
        return getMap().getEntities(Switch.class).stream().allMatch(s -> s.isActivated());
    }

    public boolean hasCollectedEnoughTreasure(int target) {
        return getInitialTreasureCount() - getMap().getEntities(Treasure.class).size() >= target;
    }
    ...
    public boolean gameCanMoveTo(Entity entity, Position position) {
        return map.canMoveTo(entity, position);
    }

    public boolean hasDestroyedAllSpawners() {
        return map.getEntities(ZombieToastSpawner.class).isEmpty();
    }

    public boolean hasDestroyedEnemies(int numEnemies) {
        return getNumEnemyKilled() >= numEnemies;
    }

    public boolean isAdjacentToPlayer(Entity entity) {
        return Position.isAdjacent(entity.getPosition(), getPlayer().getPosition());
    }

    public int getZombieToastCount() {
    ...
    public void incrementNumEnemyKilled(int delta) {
        this.setNumEnemyKilled(this.getNumEnemyKilled() + delta);
    }

    public boolean hasSwarmp() {
        return !map.getEntities(SwampTile.class).isEmpty();
    }
}
