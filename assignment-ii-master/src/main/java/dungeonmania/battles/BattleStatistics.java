...
public class BattleStatistics {
    ...
    public static List<BattleRound> battle(BattleStatistics self, BattleStatistics target) {
        ...
        while (self.getHealth() > 0 && target.getHealth() > 0) {
            ...
            double damageOnTarget = getDamageOnTarget(self, target);
            // double damageOnTarget = self.getMagnifier() * (self.getAttack() -
            // target.getDefence())
            // / target.getReducer();
    ...
    public static double getDamageOnTarget(BattleStatistics self, BattleStatistics target) {
        if (target.getHydra() == null) {
            return self.getMagnifier() * (self.getAttack() - target.getDefence())
                    / target.getReducer();
        } else {
            double lottery = Math.random();
            if (lottery <= target.getHydra().getHealthIncreaseRate() && lottery >= 0) {
                return target.getHydra().getHealthIncreaseAmount();
            } else {
                return self.getMagnifier() * (self.getAttack() - target.getDefence())
                        / target.getReducer();
            }
